...
-- deriving (Eq, Show), is called the deriving clause; 
-- it specifies that we want the compiler to automatically generate instances of the Eq and Show 
-- classes for our Employee type.
...
deptEmployees [] _ = []
deptEmployees empls dept = 
  if emplDept (head empls) == dept
  then [head empls] ++ deptEmployees (tail empls) dept
  else deptEmployees (tail empls) dept
...
comprDeptEmployees empls dept = [x | x <- empls, emplDept x == dept]
...
charIndexes string ch = [snd x | x <- zip string [0..(length string)], fst x == ch]
