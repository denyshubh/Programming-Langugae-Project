...
dept_employees1_tail(Dept, [Emp|Emps], Agg) -> 
  case Emp#employee.dept =:= Dept of
    ...
    false -> dept_employees1_tail(Dept, Emps, Agg)
  end.
...
dept_employees2(Dept, Employees) -> lists:filter(fun (Emp) -> Emp#employee.dept =:= Dept end, Employees).
...
% [expression || Pattern <- List, Conditions... ]
dept_employees3(Dept, Employees) -> [X || X <- Employees, X#employee.dept =:= Dept].
