...
dept_employees([],_,[]).
dept_employees([E|Es], Dept, [E|Os]):-
    E = employee(_,_,Dept,_),
    dept_employees(Es, Dept, Os).

dept_employees([E|Es],Dept,Os):-
    \+ E = employee(_,_,Dept,_),
    dept_employees(Es,Dept,Os).
...
list_access([I|_], List, nil) :-
	length(List, N),
	I >= N.
% Recursively calling the list_access method
list_access([I|Is], List, Z):-
	nth0(I, List, Ele),
	list_access(Is, Ele, Z).
...
divisible_by(Ints, N, Int) :- member(X,Ints), X mod N=:=0.
